...
namespace Sift.UnitTests.Architecture;

/// <summary>
/// PLAN.md §2 states Sift.Domain has zero dependencies, and §2's reference graph is what
...
public class RepositoryLayoutTests
{
    private static string RepoRoot()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "Sift.sln")))
            dir = dir.Parent;
    ...
    {
        var csproj = Project("src/Sift.Domain/Sift.Domain.csproj");
        csproj.Descendants("PackageReference").Should().BeEmpty(
            "PLAN.md §2 specifies Sift.Domain as having zero dependencies");
    }

    [Fact]
    public void Domain_has_no_project_dependencies()
    {
        ReferencedProjects(Project("src/Sift.Domain/Sift.Domain.csproj"))
    ...
    {
        ReferencedProjects(Project("src/Sift.Application/Sift.Application.csproj"))
            .Should().BeEquivalentTo(["Sift.Domain"]);
    }

    [Fact]
    public void Ai_does_not_reach_into_Infrastructure()
    {
        // §5.1: everything downstream of the validator is deterministic. Sift.Ai must not
        // acquire a route to the database, or "AI at the edge" stops being enforceable.
        ReferencedProjects(Project("src/Sift.Ai/Sift.Ai.csproj"))
            .Should().NotContain("Sift.Infrastructure");
    }
    ...
    public void Domain_and_Application_do_not_reference_EntityFrameworkCore()
    {
        foreach (var proj in new[]
                 {
                     "src/Sift.Domain/Sift.Domain.csproj",
                     "src/Sift.Application/Sift.Application.csproj",
                 })
        {
            Project(proj).Descendants("PackageReference")
                .Select(e => e.Attribute("Include")?.Value ?? string.Empty)
                .Should().NotContain(n => n.StartsWith("Microsoft.EntityFrameworkCore"),
                    "persistence concerns belong in Sift.Infrastructure (§2)");
        }
    }
}
